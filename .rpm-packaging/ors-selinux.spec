...
%define rpm_ors_temp_home_state %{rpm_state_dir}/openrouteservice-jws6-temp-home-state
%define ors_temporary_files_location %{rpm_state_dir}/install
Name: openrouteservice-jws6-selinux
Version: %{ors_version}
Release: 1
Summary: Apply SELinux rules for openrouteservice-jws6
License: GPL 3
BuildArch: noarch
Requires:  openrouteservice-jws6 = %{ors_version}
Requires:  jws6-tomcat-selinux
Vendor: HeiGIT gGmbH
...
# All variables are saved in the %{rpm_ors_temp_home_state} file for later stages.
###############################################################################################################
...
if [ -n "${ORS_HOME}" ]; then
    ...
    echo "ORS_HOME=${ORS_HOME}" > %{rpm_ors_temp_home_state}
else
...
echo "jws_webapps_folder=${jws_webapps_folder}" > ${ORS_HOME}/.openrouteservice-jws6-permanent-state
echo "jws_config_location=${jws_config_location}" >> ${ORS_HOME}/.openrouteservice-jws6-permanent-state

%preun

%post

# Source the openrouteservice-jws6-permanent-state file from %{rpm_state_dir}
. %{rpm_ors_temp_home_state}
# Source the openrouteservice-jws6-permanent-state file from ${ORS_HOME} to get the permanent variables
. ${ORS_HOME}/.openrouteservice-jws6-permanent-state

# copy permissions of jws tomcat folder to openrouteservice home folder
semanage fcontext -a -t jws6_tomcat_var_lib_t ${ORS_HOME}
