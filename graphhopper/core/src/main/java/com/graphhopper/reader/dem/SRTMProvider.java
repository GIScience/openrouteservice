...
public class SRTMProvider implements ElevationProvider
{
	...
	private void updateHeightsFromZipFile( String fileDetails, DataAccess heights ) throws RuntimeException
	{
		...
		{
			throw new RuntimeException(fileDetails, ex);
		}
	}
	...
	private byte[] getByteArrayFromZipFile( String fileDetails ) throws InterruptedException, FileNotFoundException, IOException
	{
		...
		// Runge: java.io.EOFException: Unexpected end of ZLIB input stream
		//-- Sometimes ZLIB unexpected EOF error occur.--//
		//-- We don't want to exit because of it. --//
		try {
			//while ((len = buff.read(buffer)) > 0) {
			int bytesLeft = (int)entry.getSize(); // Runge
			while (bytesLeft>0 && (len = buff.read(buffer, 0, Math.min(0xFFFF, bytesLeft))) != -1) {
				os.write(buffer, 0, len);
			}
		} catch (IOException ex) {
			logger.warn("cannot read zip archive " + fileDetails); // Runge
			throw new IOException("Cannot read zip archive " + fileDetails + ". " + ex.getMessage());
		}
