...
public class AStarBidirection extends AbstractBidirAlgo
{
    ...
    @Override
    protected Path createAndInitPath(double maxSpeed)
    {
        bestPath = new PathBidirRef(graph, flagEncoder, maxSpeed);
        return bestPath;
    }
                            ...
                            TIntObjectMap<AStarEdge> shortestWeightMap, EdgeExplorer explorer, boolean reverse )
    {
        ...
        while (iter.next())
        {
            ...
            float alreadyVisitedWeight = (float) (weighting.calcWeight(iter, reverse, currEdge.originalEdge) // Runge: substitute  currEdge.edge for currEdge.originalEdge
