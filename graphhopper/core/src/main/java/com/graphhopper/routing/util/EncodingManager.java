...
public class EncodingManager
{
        ...
        for (String entry : entries)
        {
            ...
            if (entry.equals(CAR))
            	...
            	try
            	{
            	   ...
            	   Object object = cons.newInstance(entryVal);
            	   fe = (AbstractFlagEncoder) object;
            	}
            	catch(Exception ex)
            	{
            		throw new IllegalArgumentException("Unable to create encoder " + entry +". " + ex.getMessage());
            	}
            }
    ...
    private FlagEncoder getEncoder( String name, boolean throwExc )
    {
        ...
        {
         	// System.out.println("EncodingManager.getEncoder(), name="+name+", encoder="+encoder.toString());
    ...
    public long setSidewalkSpeed(long wayFlags) {
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
    		wayFlags = encoder.setSidewalkSpeed(wayFlags);
        }
    	return wayFlags;
    }
    
    public long setNonSidewalkSpeed(long wayFlags) {
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
    		wayFlags = encoder.setNonSidewalkSpeed(wayFlags);
        }
    	return wayFlags;
    }
    ...
    public boolean isKerb(long nodeFlags) {
    	boolean isKerb = false;
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
            isKerb |= encoder.isBool(nodeFlags, FlagEncoder.K_KERB);
        }
    	return isKerb;
    }
    
    public boolean isBarrier(long nodeFlags) {
    	boolean isBarrier = false;
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
    		isBarrier |= encoder.isBool(nodeFlags, FlagEncoder.K_BARRIER);
        }
    	return isBarrier;
    }
    
    public boolean isForward(long nodeFlags) {
    	boolean isForward = false;
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
    		isForward |= encoder.isBool(nodeFlags, FlagEncoder.K_FORWARD);
        }
    	return isForward;
    }
    
    public boolean isBackward(long nodeFlags) {
    	boolean isBackward = false;
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
    		isBackward |= encoder.isBool(nodeFlags, FlagEncoder.K_BACKWARD);
        }
    	return isBackward;
    }
    
    public boolean isCrossing(long nodeFlags) {
    	boolean isCrossing = false;
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
            isCrossing |= encoder.isBool(nodeFlags, FlagEncoder.K_CROSSING);
        }
    	return isCrossing;
    }

    public long setSidewalk(long wayFlags, boolean hasSidewalk) {
    	for (AbstractFlagEncoder encoder : edgeEncoders)
        {
            if (encoder.toString().equalsIgnoreCase(WHEELCHAIR)) {
            	return encoder.setBool(wayFlags, FlagEncoder.K_SIDEWALK, hasSidewalk);
            }
        }
    	return wayFlags;
    }
    ...
    public boolean hasSidewalk(long wayFlags) {
    	boolean hasSidewalk = false;
    	for (AbstractFlagEncoder encoder : edgeEncoders) {
            hasSidewalk |= encoder.isBool(wayFlags, FlagEncoder.K_SIDEWALK);
        }
    	return hasSidewalk;
    }
