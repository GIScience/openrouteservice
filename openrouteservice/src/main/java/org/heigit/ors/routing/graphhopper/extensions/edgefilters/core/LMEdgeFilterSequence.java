...
public class LMEdgeFilterSequence extends EdgeFilterSequence implements EdgeFilter {
	...
	private boolean isAvoidCountry(String countries){
		...
		for (EdgeFilter edgeFilter: filters) {
			if (edgeFilter instanceof AvoidBordersCoreEdgeFilter avoidBordersCoreEdgeFilter){
				//There are no countries queried, but there are some in the lmset
				if(queryCountries.isEmpty())
					return false;
				return queryCountries.containsAll(Arrays.stream(avoidBordersCoreEdgeFilter.getAvoidCountries()).boxed().collect(Collectors.toList()));
			}
		}
	...
	private boolean isAvoidFeature(int avoidable){
		for (EdgeFilter edgeFilter: filters) {
			...
			if (edgeFilter instanceof AvoidFeaturesCoreEdgeFilter avoidFeaturesCoreEdgeFilter){
				...
				int filterFeatures = avoidFeaturesCoreEdgeFilter.getAvoidFeatures();
