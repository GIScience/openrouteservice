...
public class UberTrafficGraphStorage implements GraphExtension {
    ...
    private static final int LOCATION_TRAFFIC_PRIORITY = 0;         // byte location of the from heretraffic link id
    private static final int LOCATION_TRAFFIC = 1;         // byte location of the heretraffic link id
    private static final int LOCATION_TRAFFIC_MAXSPEED = 15; // byte location of the weekly maximum heretraffic speed
    private static final int FORWARD_OFFSET = 0;
    private static final int LOCATION_FORWARD_TRAFFIC_PRIORITY = LOCATION_TRAFFIC_PRIORITY + FORWARD_OFFSET;         // byte location of the from heretraffic link id
    private static final int LOCATION_FORWARD_TRAFFIC = LOCATION_TRAFFIC + FORWARD_OFFSET;         // byte location of the from heretraffic link id
    ...
    public static final int MAX_DAILY_TRAFFIC_SPEED_BYTE_COUNT = 1; // Maximum over daily heretraffic pattern values
    ...
    private int maxEdgeId = 0; // highest edge id for which heretraffic data is available
    private int patternCount; // number of heretraffic patterns
     ...
     * <p>
     * This method takes the ID of the heretraffic edge and adds the weekday specific pattern Id to the lookup.
     *
     * @param baseNode  Bade id to matchc the pattern on.
     * @param patternId Id of the heretraffic pattern.
     * @param weekday   Enum value for the weekday the heretraffic pattern Id refers to.
     **/
    ...
    /**
     * Store the heretraffic pattern for each 15 minutes for 24 hours.</-><br/><br/>
     * <p>
    ...
    /**
     * Receive the last saved heretraffic information priority for a certain ors edge id with its direction.</-><br/><br/>
     * <p>
     * This method returns the last stored priority. This is a custom value to decide if present weekly heretraffic pattern can be overwritten for a certain ors edge.
     * Traffic edges don't necessarily have the same length as the ors edges.
     * Therefore it might happen in the match making process that the same ors edge is matched multiple times on different heretraffic edges.
     ...
     * <p>
     * e.g. This function can be used to retrieve the stored length of the last ors edge <-> heretraffic edge match.
     *
     * @param edgeId   Internal ID of the graph edge.
     * @param baseNode Value of the base Node of the edge.
     * @param adjNode  Value of the adjacent Node of the edge.
    ...
    /**
     * Receive the correct heretraffic pattern for the desired hour.</-><br/><br/>
     * <p>
     * This method returns the heretraffic patterns for one hour in 15 minutes steps in a byte[].
     *
     ...
     * to find the appropriate heretraffic information.
     * Only the weekday, hour, minute are taken into consideration since the heretraffic information are generalized to that resolution at the moment.
     * <p>
     * ## Time decoding ##
     * The unix time is decoded using GMT+1 for german local time.
     * For the time being we will use the german UTC until more heretraffic data comes in.
     * <p>
     * <p>
     * ## TODO's ##
