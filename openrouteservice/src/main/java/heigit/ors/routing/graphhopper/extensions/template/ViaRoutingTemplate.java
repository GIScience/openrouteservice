 ...
 */
package heigit.ors.routing.graphhopper.extensions.template;
 ...
 * @author Peter Karich
 */
public class ViaRoutingTemplate extends com.graphhopper.routing.template.ViaRoutingTemplate {
    ...
    public List<QueryResult> lookup(List<GHPoint> points, double[] radiuses, FlagEncoder encoder, ByteArrayBuffer byteBuffer) {
        ...
        for (int placeIndex = 0; placeIndex < points.size(); placeIndex++) {
            ...
            if (ghRequest.hasPointHints()) {
                res = locationIndex.findClosest(point.lat, point.lon,
                        new NameSimilarityEdgeFilter(edgeFilter, ghRequest.getPointHints().get(placeIndex)));
                if (!res.isValid()) {
        ...
        for (int placeIndex = 1; placeIndex < pointCounts; placeIndex++) {
            ...
            if (tmpPathList.isEmpty())
                throw new IllegalStateException(
                        "At least one path has to be returned for " + fromQResult + " -> " + toQResult);

            int idx = 0;
            for (Path path : tmpPathList) {
                if (path.getTime() < 0)
                    throw new RuntimeException("Time was negative " + path.getTime() + " for index " + idx
                            + ". Please report as bug and include:" + ghRequest);
...
//                throw new Exception(
//                        "No path found due to maximum nodes exceeded " + algoOpts.getMaxVisitedNodes());
    ...
    public boolean isReady(PathMerger pathMerger, PathProcessingContext pathProcCntx) {
        if (ghRequest.getPoints().size() - 1 != pathList.size())
            throw new RuntimeException("There should be exactly one more points than paths. points:"
                    + ghRequest.getPoints().size() + ", paths:" + pathList.size());
