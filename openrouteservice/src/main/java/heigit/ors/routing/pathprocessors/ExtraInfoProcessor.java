...
public class ExtraInfoProcessor extends PathProcessor {
	...
	public void processEdge(EdgeIteratorState edge, boolean isLastEdge, PointList geom) {
		...
		{
			WaySurfaceDescription wsd = _extWaySurface.getEdgeValue(EdgeIteratorState.getOriginalEdge(edge), buffer);
		...
		{
			int value = _extWayCategory.getEdgeValue(EdgeIteratorState.getOriginalEdge(edge), buffer);
		...
		if (_trailDifficultyInfoBuilder != null)
		{
			...
			if (RoutingProfileType.isCycling(_profileType))
			{
				...
				if (_extHillIndex != null)
				{
					...
					int hillIndex = _extHillIndex.getEdgeValue(EdgeIteratorState.getOriginalEdge(edge), revert, buffer);
					if (hillIndex > 0)
						uphill = true;
				}
				
				value = _extTrailDifficulty.getMtbScale(EdgeIteratorState.getOriginalEdge(edge), buffer, uphill);
			}
			else if (RoutingProfileType.isWalking(_profileType))
				value = _extTrailDifficulty.getHikingScale(EdgeIteratorState.getOriginalEdge(edge), buffer);
		...
		{
			int value = _tollwayExtractor.getValue(EdgeIteratorState.getOriginalEdge(edge));
		...
		if (_greenInfoBuilder != null) {
			int value = _extGreenIndex.getEdgeValue(EdgeIteratorState.getOriginalEdge(edge), buffer);
		...
		if (_noiseInfoBuilder != null) {
			int noise_level = _extNoiseIndex.getEdgeValue(EdgeIteratorState.getOriginalEdge(edge), buffer);
