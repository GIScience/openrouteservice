...
public class RouteRequestHandlerTest {
    ...
    void convertRouteRequestTest() throws Exception {
        ...
        assertTrue(routingRequest.getSearchParameters().getFlexibleMode());
        ...
        while (iter.hasNext() && (weighting = iter.next()) != null) {
            if (weighting.getName().equals("green")) {
                assertEquals(0.5, weighting.getParameters().getDouble("factor", -1), 0);
            }
            if (weighting.getName().equals("quiet")) {
                assertEquals(0.2, weighting.getParameters().getDouble("factor", -1), 0);
            }
            if (weighting.getName().equals("steepness_difficulty")) {
                assertEquals(3, weighting.getParameters().getInt("level", -1), 0);
            }
        }
    }
    ...
    void testBearings() throws StatusCodeException {
        ...
        assertEquals(10.0, bearings[0].getDeviation(), 0);
        assertEquals(260.0, bearings[1].getValue(), 0);
        assertEquals(90.0, bearings[1].getDeviation(), 0);
        assertEquals(45.0, bearings[2].getValue(), 0);
        assertEquals(30.0, bearings[2].getDeviation(), 0);
    }
