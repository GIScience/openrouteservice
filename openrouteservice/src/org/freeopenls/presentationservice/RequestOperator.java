 ...
 * request is send up to the PresentationListener </p>
 *
...
public class RequestOperator {
     ...
     * send, if it is a XLSDoc, the doc to the PresentationListener(). Return the response
     * of the PresentationListener().
     * 
     * @param sRequest
     *			String that contains the XMLRequest
     * @return RespXLSDoc
     * 			- Returns Response XLSDocument from the PresentationListener()
     */
    public ResponseXLSDocument doOperation(String sRequest) {
        ...
        try{
        	...
        	mLogger.error("doOperation() - Request is NOT well-formed! -");
        ...
        if (doc instanceof XLSDocument) {
        	...
        	response = listener.doRequest(doc);
        }else{
        	mLogger.error("doOperation() - Request is NOT a XLSDoc! -");
        }
