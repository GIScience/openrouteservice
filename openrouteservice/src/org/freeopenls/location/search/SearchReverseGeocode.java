...
/**
 * <p><b>Title: SearchReverseGeocode</b></p>
 ...
 */
public class SearchReverseGeocode {
	/** Logger, used to log errors(exceptions) and additionally information */
	private final Logger mLogger = Logger.getLogger(SearchReverseGeocode.class.getName());
	 ...
	 */
	public SearchReverseGeocode(ConnectionParameter connectionParameter, PGConnection connection){
	...
	public ReverseGeocodedLocationType[] SearchInDB(String sPG_Address_SRS, String sResponseSRS, Coordinate cLocation, int maximumResponses)throws ServiceError{
	    ...
	    mLoggerCounter.info("ReverseGeocode;;;Search ReverseGeocode;"+cLocation);
		
		try {
			String query = "select *, ST_AsText(the_geom) as geom from reverse_geocode('"+cLocation.x+" "+cLocation.y+"');";				
	...
	private void addLocation(ResultSet resultSet, String PG_Address_SRS, String responseSRS, Coordinate location)throws Exception{
		...
		ResultAddress address = new ResultAddress(resultSet.getString("countrycode"), resultSet.getString("countrys"),
				...
				resultSet.getString("strname"), resultSet.getString("housenr"), null, 
				Double.parseDouble(resultSet.getString("geocode_quality")), coordinate);
