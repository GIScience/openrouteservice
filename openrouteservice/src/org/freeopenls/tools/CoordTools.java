...
public class CoordTools {
	...
	public static String getDirectionByAngle(Coordinate c1, Coordinate c2, String[] headings) {

		// double north=30,northeast=60,east=120,
		// eastsouth=150,south=210,southwest=240,west=300,northwest=330;
		double north = 0.5235987755982988, northeast = 1.0471975511965976, east = 2.0943951023931953, southeast = 2.6179938779914944, south = 3.6651914291880923, southwest = 4.1887902047863905, west = 5.235987755982989, northwest = 5.759586531581287;
		...
		if (c1.x < c2.x && c1.y < c2.y) { // I. Quadrant
			t = Math.atan(dX / dY);
			if (t < north)
				return headings[0]; //"North";
			else if (t < northeast)
				return headings[1]; //"Northeast";
			else if (t < east)
				return headings[2]; //"East";
			else
				return "unknown";
		} else if (c1.x > c2.x && c1.y < c2.y) { // II. Quadrant
			t = 2 * Math.PI - Math.atan(dX / dY);
			if (t < west)
				return headings[6]; //"West";
			else if (t < northwest)
				return headings[7]; //"Northwest";
			else if (t < north)
				return "North";
			else
				return "unknown";
			...
			t = Math.PI + Math.atan(dX / dY);
			if (t < south)
				return headings[4]; //"South";
			else if (t < southwest)
				return headings[5]; //"Southwest";
			else if (t < west)
				return headings[6]; //"West";
			else
				return "unknown";
