...
import org.freeopenls.directoryservice.documents.RequestXLSDocument;
...
/**
 * <p><b>Title: DSListener</b></p>
 * <p><b>Description:</b> Class DSListener (DirectoryServiceListener) - handles <br>
 * the DirectoryRequest to the Directory Service </p>
 *
...
public class DSListener {
    ...
    private static final Logger mLogger = Logger.getLogger(DSListener.class.getName());

    /**
     * Method that validate the request, send the DirectoryRequest(s) to RequestXLSDocument<br>
     * and return the DirectoyResponse.<br>
     ...
     * 			- Returns Response XLSDocument from the DirectoryService
     */
    public synchronized ResponseXLSDocument receiveCompleteRequest(XmlObject request) {
		...
		try {
			...
			String sResponseLanguage = DirectoryService.DEFAULT_LANGUAGE;
			
			if(xlsType.isSetLang())
				sResponseLanguage = xlsType.getLang();
			
			if (!sXLSVersion.equalsIgnoreCase(DirectoryService.SERVICE_VERSION)) {
				...
				se.addError(ErrorCodeType.OTHER_XML, XLSParameter.version.toString(),
						...
						+ " must be '"+DirectoryService.SERVICE_VERSION+"'. Delivered value was: " + sXLSVersion);
			...
			String sSrsName = DirectoryService.DEFAULT_RESPONSE_SRS;
			if(reqheaderType.isSetSrsName()){
				sSrsName = reqheaderType.getSrsName();
				if(sSrsName.equals(""))
					sSrsName = DirectoryService.DEFAULT_RESPONSE_SRS;
				else
			...
			for(int i=0; i < iNumberReceivedRequests ; i++){
				...
				requestXSLDocument.doDirectoryRequest(reqType);
			}
			...
			mLogger.error(ErrorCodeType.UNKNOWN + " OpenLS Directory Service - DSListener, ServiceError \n Message: "+ se.getMessages());
	        return new ResponseXLSDocument(se.getErrorListXLSDocument(sRequestID));
		}
		catch (Exception e) {
			ServiceError se = new ServiceError(SeverityType.ERROR);
            se.addInternalError(ErrorCodeType.UNKNOWN, "OpenLS Directory Service - DSListener, Message: ", e);
			mLogger.error(ErrorCodeType.UNKNOWN + " OpenLS Directory Service - DSListener, Exception \n Message: "+ se.getMessages());
            return new ResponseXLSDocument(se.getErrorListXLSDocument(sRequestID));
		}
