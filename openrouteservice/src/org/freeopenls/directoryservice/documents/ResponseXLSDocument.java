...
public class ResponseXLSDocument{
    ...
    public ResponseXLSDocument(String sessionID) {
		...
		if (cursorXLSDoc.toFirstChild())
			cursorXLSDoc.setAttributeText(new QName("http://www.w3.org/2001/XMLSchema-instance","schemaLocation"), OpenLS.SCHEMA_LOCATION_OPENLS+" "+DirectoryService.SCHEMA_FILENAME_DIRECTORYSERVICE);
    ...
    /**
     * Add DirectoryResponseType to the document
     * 
     * @return DirectoryResponseType
     */
    public DirectoryResponseType addResponseParameters(){
		AbstractResponseParametersType arespparamType = mResponse.addNewResponseParameters();
		return (DirectoryResponseType) arespparamType.changeType(DirectoryResponseType.type);
    }

    public void doWellFormedDirectoryResponse(){
		//For well formed XML-Doc
		...
		if (cursorReqType.toChild(new QName("http://www.opengis.net/xls", "_ResponseParameters")))
			cursorReqType.setName(new QName("http://www.opengis.net/xls","DirectoryResponse"));
		cursorXLS.dispose();
