 ...
 * @author laurent
 */
public class RecursiveGridIsolineBuilder implements AbstractIsolineBuilder {
	...
	private static class GridEdge {
		...
		public final boolean equals(Object other) {
			if (other instanceof GridEdge) {
				...
				return A.equals(otherEdge.A) && size == otherEdge.size && horizontal == otherEdge.horizontal;
			}
			return false;
		}
	...
	//	LOG.debug("Created {} dots and {} edges out of {} initial points.", initialDots.size(), initialEdges.size(),
	//			p0List.size());
	}
	...
	public Geometry computeIsoline(long z0) {
		...
		while (!finalEdges.isEmpty()) {
			...
			int cut = e0.cut(z0);
			Direction direction = e0.horizontal ? (cut > 0 ? Direction.UP : Direction.DOWN) : (cut > 0 ? Direction.LEFT
					: Direction.RIGHT);
			GridEdge e = e0;
			while (true) {
	...
	//	LOG.info("Isochrones: {}+{} Fz samples, {} cutting edges, {} non-cutting edges", allDots.size(),
	//			fzInterpolateCount, finalEdgesSize, finalNonCuttingEdgesSize);
		/*
	...
	private final List<Polygon> punchHoles(GeometryFactory geomFactory, List<LinearRing> rings) {
		...
		for (LinearRing ring : rings) {
			if (getArea(ring.getCoordinateSequence(), true) > 0.0)
				holes.add(ring);
			else
				shells.add(geomFactory.createPolygon(ring, null));
		}
    ...
    private final double getArea(CoordinateSequence coords, boolean signed)
    {
      int n = coords.size();
      if (n < 3)
        return 0;
      ...
      double sum = 0.0F;
      Coordinate p = coords.getCoordinate(0);
      double bx = p.x;
      double by = p.y;

      for (int i = 1; i < n; i++)
      {
        p = coords.getCoordinate(i);
        double cx = p.x;
        double cy = p.y;
