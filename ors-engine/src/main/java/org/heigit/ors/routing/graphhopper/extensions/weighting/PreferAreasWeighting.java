...
public class PreferAreasWeighting extends FastestWeighting {
    private Weighting superWeighting;
    private Envelope env;
    private final WeightedPolygon[] polys;
    ...
    public PreferAreasWeighting(FlagEncoder encoder, PMap map, GraphHopperStorage graphStorage) {
        super(encoder, map);
        this.polys = map.getObject("prefer_areas", null);
        ...
        if (polys != null && polys.length > 0) {
            ...
            for (WeightedPolygon poly : polys) {
    ...
    public final double getFactor(EdgeIteratorState iter) {
        if (env == null)
        ...
        if (inEnv || !(eMinX > env.getMaxX() || eMaxX < env.getMinX() || eMinY > env.getMaxY() || eMaxY < env.getMinY())) {
            ...
            if (size >= 2) {
                ...
                for (int i = 0; i < polys.length; i++) {
                    WeightedPolygon poly = polys[i];
                    if (poly.contains(ls) || ls.crosses(poly)) {
                        return poly.getWeightingFactor();
                    }
                }
            } else {
                return DEFAULT_FACTOR;
...
//            return Double.POSITIVE_INFINITY;
        ...
        //Get polygon factor here
        return getFactor(edgeState);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        final PreferAreasWeighting other = (PreferAreasWeighting) obj;
