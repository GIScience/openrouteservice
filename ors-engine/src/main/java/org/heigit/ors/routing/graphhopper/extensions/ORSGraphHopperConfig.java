...
public class ORSGraphHopperConfig extends GraphHopperConfig {
    ...
    private static void setGraphLevelEncodedValues(BuildProperties buildProperties, ORSGraphHopperConfig ghConfig) {
        ...
        Map<String, ExtendedStorageProperties> extStorages = buildProperties.getExtStorages();
        if (extStorages != null && extStorages.containsKey(ExtendedStorageName.WAY_SURFACE_TYPE.getValue())) {
            extStorages.remove(ExtendedStorageName.WAY_SURFACE_TYPE.getValue());
            encodedValues.add(WaySurface.KEY);
            encodedValues.add(WayType.KEY);
        }
