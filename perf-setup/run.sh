...
do
  ...
  for i in $(seq 1 $max_iteration); do
    ...
    ch_threads_car="ors.engine.profiles.bike-regular.preparation.methods.ch.threads=${thread}"
    fastisochrones_threads="ors.engine.profile_default.preparation.fastisochrones.threads=${thread}"
    fastisochrones_threads_car="ors.engine.profiles.car.preparation.methods.fastisochrones.threads=${thread}"
    docker run -it --user $UID -d -p 8082:8082 --env-file ./config.env -e "${fastisochrones_threads_car}" -e "${ch_threads_car}" -v ./heidelberg.osm.gz:/home/ors/files/osm.pbf:ro --name ors-instance openrouteservice/openrouteservice:v8.1.0
