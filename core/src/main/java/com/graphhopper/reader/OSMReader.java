...
public class OSMReader implements DataReader
{
    ...
    // pillar node is >= 3
    // https://lists.openstreetmap.org/pipermail/graphhopper/2014-December/001423.html
    ...
    // tower node is <= -3
    /**
     * Vertex at junction (=node in graph theory)
     */
    ...
    protected EncodingManager encodingManager = null; // runge protected
            ...
            while ((item = in.getNext()) != null)
            {
                if (item.isType(OSMElement.WAY))
                {
                    ...
                    if (valid)
                    {
                        ...
                        {
                          	// System.out.println("OSMReader.preProcess(), wayName="+way.getTag("name")+", "+index+", node="+wayNodes.get(index));
    ...
    protected void processWay( OSMWay way ) 
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            if (nodeFlags > 0)
            {
                if ((nodeFlags & wayFlags) > 0 || isBarrier(way, nodeFlags, wayFlags))
                {
                    // remove barrier to avoid duplicates
                	// exclude kerb nodes, since in case they are crossings at the same time the node flags are still needed in a later processing step
        ...
        try {
        	...
        	Collection<EdgeIteratorState> sidewalkEdges = addSidewalks(way, wayFlags);
        	
        	// reduce speed for edges of way feature that have a derived sidewalk to force usage of these sidewalks
        	if (sidewalkEdges != null && way.containsTag("highway") && way.containsTag("sidewalk")) {
        		for (EdgeIteratorState wayEdge : createdEdges) {
        			long tempWayFlags = wayEdge.getFlags();
					tempWayFlags = encodingManager.setNonSidewalkSpeed(tempWayFlags);
					wayEdge.setFlags(tempWayFlags);
				}
        	}
        	
        	if (sidewalkEdges != null)
        		createdEdges.addAll(sidewalkEdges);
        }
    ...
    public void processRelation( OSMRelation relation ) throws XMLStreamException
    {
        if (relation.hasTag("type", "restriction"))
        {
            ...
            if (turnRelation != null)
            {
                ...
                TurnCostExtension tcs = Helper.getTurnCostExtensions(extendedStorage); // Runge
                if (tcs != null)
                {
    ...
    protected double getTmpElevation( int id )
    {
        if (id == EMPTY)
            return Double.NaN;
        if (id < TOWER_NODE)
        {
            // tower node
            id = -id - 3;
            return nodeAccess.getElevation(id);
        } else if (id > -TOWER_NODE)
        {
            // pillar node
            id = id - 3;
            return pillarInfo.getElevation(id);
        } else
            // e.g. if id is not handled from preparse (e.g. was ignored via isInBounds)
            return Double.NaN;
    }
    ...
    protected boolean addNode( OSMNode node )
    {
        ...
        // System.out.println("OSMReader.addNode(), node.getId()="+node.getId()+", nodeType="+nodeType);
    ...
    protected void prepareHighwayNode( long osmId )
    {
        int tmpIndex = getNodeMap().get(osmId);
        // System.out.println("OSMReader.prepareHighwayNode(), tmpIndex="+tmpIndex+", osmId="+osmId);
    ...
    protected int addTowerNode( long osmId, double lat, double lon, double ele )
    {
        if (nodeAccess.is3D())
        ...
        // System.out.println("OSMReader.addTowerNode(), osmId="+osmId+", nextTowerId="+nextTowerId+", id="+id);
    ...
    public Collection<EdgeIteratorState> addOSMWay( final TLongList osmNodeIds, final long flags, final long wayOsmId )
    {
        ...
        // System.out.println("OSMReader.addOSMWay(), osmNodeIds.size()="+osmNodeIds.size());
        ...
        try
        {
            for (int i = 0; i < osmNodeIds.size(); i++)
            {
                ...
                // System.out.println("OSMReader.addOSMWay(), osmId="+osmId+", tmpNode="+tmpNode+", wayOsmId="+wayOsmId);
                if (tmpNode == EMPTY)
                    continue;
    ...
    protected long createNewNodeId()
    {
    ...
    //Runge
    public PointList getWayPoints(OSMWay way)
    {
    	TLongList osmNodeIds = way.getNodes();
    	PointList pointList = new PointList(osmNodeIds.size(), nodeAccess.is3D());
...
//                            newEdges.add(addEdge(firstNode, tmpNode, pointList, flags, wayOsmId));
  //                          pointList.clear();
    ...
    /**
     * Serves as a hook that is overridden in inheriting classes.
     * Used to generate sidewalks for pedestrian and wheelchair routing profiles.
     * 
     * @param way
     * @param wayFlags
     * @return
