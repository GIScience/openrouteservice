...
public class FastestWeighting implements Weighting
{
    ...
    public FastestWeighting(double userMaxSpeed, FlagEncoder encoder)
    {
    	this(encoder);
        
        if (userMaxSpeed > 0)
        {
        	this.userMaxSpeed = userMaxSpeed; // Runge
        	maxSpeed = userMaxSpeed*SPEED_CONV;
        }
        else
        {
        	maxSpeed = encoder.getMaxSpeed()*SPEED_CONV;
    ...
    public double calcWeight( EdgeIteratorState edge, boolean reverse, int prevOrNextEdgeId )
    {
        ...
        if (userMaxSpeed > 0)
        {
        	if (speed > userMaxSpeed) // Runge
        		speed = userMaxSpeed;
        	else if (speed == defaultMaxSpeed && userMaxSpeed > speed)
        	{
        		speed = userMaxSpeed;
        	}
        }
