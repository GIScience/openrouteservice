...
//                ? TraversalMode.EDGE_BASED_2DIR : TraversalMode.NODE_BASED;                
     ...
     */
    public MatchResult doWork(List<GPXEntry> gpxList, EdgeFilter edgeFilter) {
        int currentIndex = 0;
        if (gpxList.size() < 2) {
        ...
        while (true) {
            ...
            MatchResult subMatch = doWork(firstQueryResults, gpxSublist, gpxLength, doEnd, edgeFilter);
    ...
    MatchResult doWork(List<QueryResult> firstQueryResults,
            List<GPXEntry> gpxList, double gpxLength, boolean doEnd, EdgeFilter edgeFilter) {
    ...
    private class CustomDijkstra extends Dijkstra {
        ...
        @Override
        public void runAlgo(double maxSpeed) {
            checkAlreadyRun();
            super.runAlgo(maxSpeed);
        }
        ...
        public Path extractPath(Collection<QueryResult> endQRs) {
            ...
            return new Path(graph, flagEncoder, -1).setWeight(currEdge.weight).setEdgeEntry(currEdge).extract();
        }
    }
