...
public class GraphHopper implements GraphHopperAPI
{
	...
	public GraphHopper init( CmdArgs args )
	{
		...
		preparationThreads = args.getInt("prepare.threads", 1);
	...
	public void initCHAlgoFactories()
	{
		if (algoFactories.isEmpty())
			...
			{
				Weighting weighting = createWeighting(new WeightingMap(chWeightingStr), -1,  encoder, null); // Runge
	...
	public Weighting createWeighting( WeightingMap weightingMap,FlagEncoder encoder) // Runge
	{
		return createWeighting(weightingMap, -1, encoder, null);
	}
	 ...
	 * @see WeightingMap
	 */
	public Weighting createWeighting( WeightingMap weightingMap, double maxSpeed, FlagEncoder encoder, GraphStorage graphStorage)
	{
	...
	{
		if (!(weighting instanceof TurnWeighting)) // Runge
		{
	...
	protected List<Path> getPaths( GHRequest request, GHResponse rsp,  ArrayBuffer arrayBuffer)
	{
		...
		boolean flexibleMode = request.getHints().getBool("CH.Disable", false); // Runge

		if (chEnabled && !flexibleMode)
		{
			...
			weighting = createWeighting(request.getHints(), request.getMaxSpeed(), encoder, ghStorage); // Runge
		...
		for (int placeIndex = 1; placeIndex < points.size(); placeIndex++)
		{
			...
			// Runge
			if (containsEdgeFilter(edgeFilter, DestinationDependentEdgeFilter.class))
			{
				DestinationDependentEdgeFilter ddef = (DestinationDependentEdgeFilter)getEdgeFilter(edgeFilter, DestinationDependentEdgeFilter.class);
				if (ddef != null)
					ddef.setDestinationEdge(toQResult.getClosestEdge(), ghStorage, encoder, tMode);
			}

			if (edgeFilter.getClass() != DefaultEdgeFilter.class) // Runge
			{
				AbstractRoutingAlgorithm ara = (AbstractRoutingAlgorithm)algo;
				ara.setEdgeFilter(edgeFilter);
			}

			sw = new StopWatch().start();
			Path path = algo.calcPath(fromQResult.getClosestNode(), toQResult.getClosestNode(), request.getMaxSpeed());
			if (path.getTime() < 0)
	...
	// Runge
	private boolean containsEdgeFilter(EdgeFilter edgeFilter, Class<?> type)
	{
		boolean res = false;

		if (type.isAssignableFrom(edgeFilter.getClass()))
			res = true;
		else if (edgeFilter instanceof EdgeFilterSequence)
		{
			EdgeFilterSequence seq = (EdgeFilterSequence)edgeFilter;
			res = seq.containsEdgeFilter(type);
		}
	...
	// Runge
	private EdgeFilter getEdgeFilter(EdgeFilter edgeFilter, Class<?> type)
	{
		if (type.isAssignableFrom(edgeFilter.getClass()))
			return edgeFilter;
		else if (edgeFilter instanceof EdgeFilterSequence)
		{
			EdgeFilterSequence seq = (EdgeFilterSequence)edgeFilter;
			return seq.getEdgeFilter(DestinationDependentEdgeFilter.class);
		}
		...
		if (tmpPrepare)
		{
			...
			if (preparationThreads <= 1)
			{
				for (Entry<Weighting, RoutingAlgorithmFactory> entry : algoFactories.entrySet()) {
					...
					PrepareContractionHierarchies item =  ((PrepareContractionHierarchies) entry.getValue());
					logger.info((++counter) + "/" + algoFactories.size() + " calling CH prepare.doWork for " + entry.getKey()  + " ... (" + Helper.getMemInfo() + ")");
			...
			{
				ExecutorService threadPool  = Executors.newFixedThreadPool(preparationThreads);
				...
				for (Entry<Weighting, RoutingAlgorithmFactory> entry : algoFactories.entrySet()) {
					if (!(entry.getValue() instanceof PrepareContractionHierarchies))
			         ...
			         completionService.submit(new Runnable() {
			                ...
			                public void run() {
			                    try {
			                        Thread.currentThread().setName("ORS-chb-" + name);
			                        item.doWork();
			                    } catch (Exception ex) {
				...
				try {
					for (int i = 0; i < algoFactories.size(); i++) {
