...
configWithNonExistingElevationCacheFolder=$(makeTempFile $(basename $0) '{
  "ors": {
    "info": {
      ...
      "routing": {
        ...
        "profiles": {
          ...
          "default_params": {
            ...
            "elevation_cache_path": "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
...
# This test should load the json config which contains a path to a non existing elevation cache folder.
...
podman run --replace --name "${CONTAINER}" -p "${HOST_PORT}":8082 \
  ...
  -v "${configWithNonExistingElevationCacheFolder}":${CONTAINER_WORK_DIR}/ors-config.json \
  --env ORS_CONFIG="${CONTAINER_WORK_DIR}/ors-config.json" \
  --env ELEVATION_CACHE_FOLDER="${CONTAINER_WORK_DIR}/elevation_cache" \
  "local/${IMAGE}:latest" &
