...
jsonConfig=$(makeTempFile $(basename $0) '{
  "ors": {
    "info": {
...
# This test should load the json config specified with the environment variable ORS_CONFIG.
# This is the way, the deprecated JSON config should still be usable
# until the support for JSON config will be removed completely.
podman run --replace --name "${CONTAINER}" -p "${HOST_PORT}":8082 \
  -v "${M2_FOLDER}":/root/.m2 \
  -v "${TESTROOT}/graphs_volume":"${CONTAINER_WORK_DIR}/graphs" \
  -v "${jsonConfig}":${CONTAINER_WORK_DIR}/my-config.json \
  --env ORS_CONFIG="${CONTAINER_WORK_DIR}/my-config.json" \
  "local/${IMAGE}:latest" &
