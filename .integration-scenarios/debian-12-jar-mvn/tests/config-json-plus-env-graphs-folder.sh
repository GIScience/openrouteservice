...
jsonConfigFileWithNonexistingGraphsFolder=$(makeTempFile $(basename $0) '{
  "ors": {
    "info": {
...
# This test should load the json config which contains a path to a non existing graphs folder.
# Without the variable GRAPHS_FOLDER, which contains a path to an existing directory,
# openrouteservice should not start up. But if GRAPHS_FOLDER is evaluated correctly,
# it should start with the expected activated profile.
podman run --replace --name "${CONTAINER}" -p "${HOST_PORT}":8082 \
  -v "${M2_FOLDER}":/root/.m2 \
  -v "${TESTROOT}/graphs_volume":"${CONTAINER_WORK_DIR}/customgraphs" \
  -v "${jsonConfigFileWithNonexistingGraphsFolder}":${CONTAINER_WORK_DIR}/ors-config.json \
  --env ORS_CONFIG="${CONTAINER_WORK_DIR}/ors-config.json" \
  --env GRAPHS_FOLDER="${CONTAINER_WORK_DIR}/customgraphs" \
  "local/${IMAGE}:latest" &
