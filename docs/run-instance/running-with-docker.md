...
This will pull openrouteservice of the selected version and start it up using an example setup
...
* `ors-config.yml`: The default config used by the openrouteservice instance inside the container. This file will not be overridden by docker. Initially, the file is identical to `example-ors-config.yml`. See [Use customized config file](#use-customized-config-file)   
* `elevation_cache`: Directory, where openrouteservice stores downloaded elevation data to avoid repeated downloads.
* `example-heidelberg.test.pbf`: The initial sample OSM data used to run the container in its initial setup. Normally,
  you will remove this file or add another OSM file to the same directory and adapt your config to use your file.
...
ors-app  | 2024-03-12T08:13:19.142118351Z ⓘ Default to example osm source file: "/home/ors/files/example-heidelberg.test.pbf"
...
ors-app  | 2024-03-12T08:13:19.199741022Z ✓ The file /home/ors/files/example-heidelberg.test.pbf is up to date
...
* The evaluated OSM file: `====> Initializing profiles from '/home/ors/files/example-heidelberg.test.pbf'` (also
  internal path)
